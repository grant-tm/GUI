...
static Rect2 GUI_LayoutNextRect (GUIContext *context, f32 height)
{
    ...
    rect.max.x = scope->content_rect.max.x;
    rect.max.y = scope->cursor.y + height;
...
void GUI_BeginPanel (GUIContext *context, GUIID id, Rect2 rect, const GUIPanelStyle *style)
{
    ...
    scope->cursor = scope->content_rect.min;
    scope->spacing = resolved_style.spacing;
}
...
void GUI_Label (GUIContext *context, String text, const GUILabelStyle *style)
{
    ...
    rect = GUI_LayoutNextRect(context, style->height);
